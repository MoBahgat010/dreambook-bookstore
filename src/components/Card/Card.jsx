...
function Card(props) {
  ...
  function handleSettingLiked() {
    ...
    else dispatch(AddThenGetWishList(props.id));
  }
  ...
  return (
    <Link
      to={`/single-page/${props.id}`}
      className="card w-100 position-relative overflow-hidden"
    >
      {props.newBadge && (
        <div className="new-badge d-flex justify-content-center align-items-center position-absolute">
          {t("New")}
        </div>
      )}
      {Boolean(props.discount) && (
        <div className="discount-badge d-flex justify-content-center align-items-center position-absolute">
          {props.discount}% {t("Off")}
        </div>
      )}
      <div className="image-container w-100">
        <img
          src={`${process.env.REACT_APP_BASE_URL}products/images/${props.image}`}
          className=""
          alt=""
        />
      </div>
      <div className="card-body text-center">
        ...
        {Boolean(props.discount) ? (
          <div className="d-flex gap-2">
            <p className="card-text old-price">
              {props.price} {countryCurrency}
            </p>
            <p className="card-text">
              {props.price * ((100 - props.discount) / 100.0)} {countryCurrency}
            </p>
          </div>
        ) : (
          <p className="card-text">
        ...
        {props.quantity == 0 && (
          <p className="m-0 p-0 card-text text-center old-price">
            out of stock
          </p>
        )}
