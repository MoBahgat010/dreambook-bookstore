...
import {
  ...
  ChangeCurrency,
  changeCurrency,
} from "../../RTK/Slices/SelectedCountrySlice";
import {
  hideSearchComponent,
  showComponents,
  showSearchComponent,
} from "../../RTK/Slices/ComponentsSlice";
...
function Header() {
  ...
  const { countryImg, countryName, countryCurrency } = useSelector(
    (state) => state.SelectCountry
  );
  const { allCategories, allSubCategories } = useSelector(
  ...
  useEffect(() => {
    // destroy is not a function is an error due to that useEffect function has to be sync not asnyc
            ...
            onClick={() => {
              dispatch(
                ChangeCountry({
                  ...
                  countryCurrency: country.currencyName,
                })
              );
            }}
            key={index}
            className="py-1 py-md-2 d-flex align-items-center justify-content-between"
          >
  ...
  function ManipulateSubDropDowns(order) {
    if (isSmallScreen) {
      if (!lastOpenedSubDropDown) ShowSubDropDowns(order);
      else if (lastOpenedSubDropDown == order) CloseSubDropDowns(order);
      else {
  ...
  function ManipulateSearchBar() {
    if (isSmallScreen) isSearchBarOpen ? CloseSearchBar() : ShowSearchBar();
  }

  function ManipulateMenuBar() {
    if (isSmallScreen) isMenuBarOpen ? CloseMenuBar() : ShowMenuBar();
  }
  ...
  useEffect(() => {
    if (isSmallScreen) CloseSearchBar();
  }, [SearchBarComponent]);
    ...
    <header ref={headerComponenet}>
      <nav className="upper-one navbar navbar-expand">
        <div className="container">
          <div
            className="collapse navbar-collapse flex-column flex-md-row d-flex justify-content-center justify-content-md-between align-items-center"
            ...
            <ul className="navbar-nav">
              <li className="nav-item d-flex align-items-center terms pe-1 p-lg-3">
                <Link
                  ...
                  className="active terms-link text-white"
                  aria-current="page"
                  href="#"
                >
                ...
                <div className="w-100 h-100 social-media d-flex align-items-center">
                  <div>
                    <Link
                      ...
                      className="d-block"
                      to={
                        "https://api.whatsapp.com/send/?phone=96551455511&text&type=phone_number&app_absent=0"
                      }
                    >
                  ...
                  <div>
                    <Link
                      ...
                      className="d-block"
                      to={"https://www.instagram.com/dreambookq8/"}
                    >
      ...
      <nav className="middle-one navbar navbar-expand bg-white">
        <div className="container d-flex justify-content-md-between justify-content-evenly">
          ...
          <div className="search-bar">
            ...
            />
            <div
              className="menu-container px-2 d-block d-md-none"
              onClick={() => {
                ManipulateMenuBar();
              }}
            >
          ...
          <div className="icons-container d-flex align-items-center justify-content-center">
            <Link
              ...
              to={"/wishlist"}
              className="d-block px-2 wishlist position-relative"
            >
              <i className="fa-regular fa-heart"></i>
              <div className="position-absolute">
                {wishproducts?.length == undefined ? 0 : wishproducts.length}
              </div>
            </Link>
            <div
              onClick={() => {
        ...
        <div className="container py-md-0 fw-md-bold d-flex flex-md-row flex-column justify-content-md-evenly align-items-start align-items-md-center">
          ...
          {categories.map(([category, subCategories], index) => {
            return (
              <div
                key={index}
                className="has-dropdown mb-md-0 position-relative px-1"
                onClick={() => ManipulateSubDropDowns(index + 2)}
              >
          ...
          {otherCategories.length != 0 && (
            <div
              className="has-dropdown others mb-md-0 position-relative px-1"
              onClick={() => ManipulateSubDropDowns(categories.length + 2)}
            >
              <div className="w-100 h-100 d-flex align-items-center">
                <Link
                  onClick={(e) => {
