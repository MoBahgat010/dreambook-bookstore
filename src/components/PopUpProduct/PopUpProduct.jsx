...
function PopUpProduct() {
  ...
  useEffect(() => {
    axios
      .get(process.env.REACT_APP_BASE_URL + "api/v1/popup")
      .then((res) => {
        const allPopupProducts = res.data.result;
        if (allPopupProducts.length > 0) {
          const chosenProduct = Math.floor(
            Math.random() * allPopupProducts.length
          );
  ...
  function ShowPopUp() {
    ...
    tl.to(".pop-up-product .inner-container", {
      opacity: 1,
      duration: 0.5,
    }).set("body", {
      overflow: "hidden",
  ...
  useEffect(() => {
    if (
      ...
      popupProduct.constructor === Object
    ) {
  ...
  if (
    (popupProduct != null || popupProduct != undefined) &&
    (popupProduct.product != null || popupProduct.product != undefined)
  )
    return (
      <div className="pop-up-product">
        <Link
          to={`/single-page/${popupProduct.product._id}`}
          onClick={HidePopUp}
        ...
        >
          ...
          <div className="product-image mb-2">
            <img
              src={`${process.env.REACT_APP_BASE_URL}products/images/${popupProduct.product.image}`}
              alt={popupProduct.product.title}
            />
          </div>
          <button
            onClick={(e) => {
    ...
    return (
      <div className="pop-up-product">
        <div className="inner-container">
          ...
          <div className="product-image mb-2">
            <img
              src={`${process.env.REACT_APP_BASE_URL}uploads/${popupProduct.image}`}
