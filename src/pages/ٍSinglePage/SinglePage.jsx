...
import {
  AddThenGetCartProducts,
  TriggerInsuffecientProductQuantity,
...
import {
  AddThenGetWishList,
  RemoveThenGetWishList,
} from "../../RTK/Slices/ProductsWishListSlice";
...
function SinglePage() {
  ...
  function handleSettingLiked() {
    ...
    else dispatch(AddThenGetWishList(params.productId));
  }
      ...
      <div className="container">
        <div className="inner-container fw-bolder row">
          <div className="image-container px-2 col-12 col-md-6">
            <img
              src={`${process.env.REACT_APP_BASE_URL}products/images/${certainProduct.image}`}
              alt=""
            />
          </div>
          <div className="data col-12 col-md-6">
            <div className="upper-data">
              ...
              {certainProduct.priceAfterDiscount != certainProduct.price ? (
                <div className="d-flex gap-2">
                  <span>
                    {certainProduct.priceAfterDiscount} {countryCurrency}
                  </span>
                  <span className="old-price">
                    {certainProduct.price} {countryCurrency}
                  </span>
                </div>
              ) : (
                <span>
                ...
                <div className="select-quantity mt-2 d-flex align-items-center">
                  <div className="d-flex pe-1">
                    <button
                      ...
                      className="text-white fs-3 py-1 px-3"
                    >
                      -
                    </button>
                    <div className="p-2 d-flex bg-light-subtle align-items-center">
                      {quantity}
                    </div>
                    <button
                      onClick={() => {
                  ...
                  <div className="px-1">
                    <button
                      ...
                      type="button"
                      className="h-100 text-white rounded-pill py-2 px-4"
                    >
                      {t("Add to Cart")}
                    </button>
                  </div>
                  {certainProduct.quantity < 10 && (
                    <p className="alert p-0 m-0 ms-2 text-danger">
                      {certainProduct.quantity} item
                      {certainProduct.quantity != 1 && <span>s</span>} available
                    </p>
                  )}
                  {certainProduct.new && (
                    <p style={{ color: "#916741" }} className="mx-2">
                      {t("New")}
                    </p>
                  )}
                </div>
            ...
            <div className="lower-data d-flex flex-column mt-3">
              <table className="table table-striped flex-grow-1 table-bordered">
                <tbody>
                  <tr>
                    ...
                    <td>
                      {i18next.language === "en"
                        ? certainProduct.category?.englishname
                        : certainProduct.category?.arabicname}
                    </td>
                  </tr>
                  <tr>
                    <td>{t("SubCategory")}</td>
                    <td>
                      {i18next.language === "en"
                        ? certainProduct.Subcategory?.englishname
                        : certainProduct.Subcategory?.arabicname}
                    </td>
                  </tr>
                  <tr>
                    <td>{t("AUTHOR")}</td>
              ...
              <div className="more-options d-flex justify-content-between">
                <div
                  className="like position-relative p-2 px-3 bg-info"
                  onClick={() => {
                    handleSettingLiked();
                  }}
                >
