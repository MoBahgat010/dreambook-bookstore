...
function Checkout() {
  ...
  useEffect(() => {
    Street.current.value =
      shippingData.street != undefined ? shippingData.street : "";
    Block.current.value =
      shippingData.building != undefined ? shippingData.building : "";
    Area.current.value =
      shippingData.area != undefined ? shippingData.area : "";
    Floor.current.value =
      shippingData.floor != undefined ? shippingData.floor : "";
    Appartement.current.value =
      shippingData.apartment != undefined ? shippingData.apartment : "";
    City.current.value =
      shippingData.city != undefined ? shippingData.city : "";
    Mobile.current.value =
      shippingData.phone != undefined ? shippingData.phone : "";
  }, [shippingData]);
        ...
        <div className="inner-container row">
          <div className="left-form col-md-4 col-12 py-4">
            <div>
              <div className="title d-flex align-items-center p-1">
                <div>
                  <div className="step-one px-3 py-2 text-white">1</div>
                </div>
          ...
          <div className="right-data col-md-8 col-12 py-4">
            <div>
              <div className="title d-flex align-items-center p-1">
                <div>
                  <div className="step-one px-3 py-2 text-white">2</div>
                </div>
                <div className="address">
                  <p>{t("CART DETAIL")}</p>
                </div>
                    ...
                    return (
                      <tr>
                        ...
                        <td className="text-center">
                          {countryCurrency} {product.price}
                        </td>
                        <td className="text-center">{product.quantity}</td>
                        <td className="text-center">
                          {countryCurrency}{" "}
                          {(product.quantity * product.priceExchanged).toFixed(
                            2
                          )}
                        </td>
              ...
              <div className="cart-total">
                ...
                <div className="p-2">
                  ...
                  <div className="shipping d-flex justify-content-between">
                    <div>{t("Shipping")}</div>
                    <div>
                      {(0.05 * cartTotal).toFixed(2)} {countryCurrency}
                    </div>
                  </div>
                  <div className="total d-flex justify-content-between">
                    <div>{t("Grand Total")}</div>
                    <div>
                      {(1.05 * cartTotal).toFixed(2)} {countryCurrency}
                    </div>
                  </div>
                </div>
              </div>
